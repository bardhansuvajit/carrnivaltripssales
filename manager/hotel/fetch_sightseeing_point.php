...
if (empty($_SESSION['sess_user'])) {
    $sqlTable = "SELECT * FROM carrnivaltrips_hotel  ";
} else {
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    $products .= '<div class="form-check form-check-inline bg-primary  m-1"  style="padding:4px 30px; border-radius:5px" ><input class="form-check-input me-2" id="sightseeing_point_'. $id .'"  type="checkbox" data-filter="sightseeing_point" value="'. $id .'"><label class="form-check-label" for="sightseeing_point_'. $id .'">'. $name .'</label></div>';
}
