...
if (empty($_SESSION['sess_user'])) {
    $sqlTable = "SELECT * FROM carrnivaltrips_sightseeing_img  ";
} else {
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    $products .= '<div class="form-check form-check-inline bg-primary  m-1 col-3"  style="padding:5px 15px 5px 30px; border-radius:5px" ><input class="form-check-input me-2" id="sightseeing_img_'. $id .'"  type="checkbox" data-filter="sightseeing_img" value="'. $id .'"><label class="form-check-label" for="sightseeing_img_'. $id .'"><img src="../../sightseeing/sightseeing_img/'. $name .'" style="width: 100%; hight:100px; border-radius: 5px; margin-top:3px;" height="80px" ></label></div>';
}
